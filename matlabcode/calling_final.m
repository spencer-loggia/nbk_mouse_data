...
%trained
EXPECTED_IDS = [{'MiTg01'}, {'MiTg15'}, {'MiTg13'}, {'MiTg04'}, {'MiTg05'}, {'MiTg14'}, {'MiTg16'}, {'MiTg08'}, {'MiTg19'}, {'MiTg10'}, {'MiTg11'}, {'MiTg12'}, {'MiTg17'}, {'MiTg18'}, {'SCB_2'}, {'SCB05'}, {'SCB06'}];
...
if strcmp(fname,'today') == 1 || (isnumeric(year_num) == 1 && L == 11)
    ...
    for w = 1:length(boxdrives)
        ...
        for x = 1:length(subfolders)
            ...
            i = 1;
            while i <= num_fname
                data = plot_session_performance_v13(fname_arr{i}, plot_flag);
                z = 1;
%                 while strcmp(data.name, char(EXPECTED_IDS(z))) == 0 && z <= 15
                while strcmp(data.name, char(EXPECTED_IDS(z))) == 0 && z <= length(EXPECTED_IDS)
                    z = z + 1;
                end
    ...
    disp(dataArray(:,1:14));
    disp('SCB data');
    disp(dataArray(:,15:17))
else
    if isnumeric(order_num) == 1 && L == 15
        plot_session_performance_v12([fname, '.csv'], plot_flag)
    elseif L == 17 || L == 18
        plot_session_performance_v12(fname, plot_flag)
    else
