...
% cd D:\Postdoc_Mysore_lab\Research_data\behavior
 function data = plot_session_performance_v10(fname,plot_flag)
%clear all; close all;
% plot_flag = 0;
...
image_name_list_lineno = 41; % Hopefully this remains constant
protocol_database_lineno = 23; % Hopefully this remains constant
...
%commented out to remove issues with textscan in matlab 2017b and later
...
subject_ID = dataArray{1,2}{linenum,1};
...
t_val = textscan(lines{1,1}{num_lines-2,1}, '%s%s%[\n\r]', 'Delimiter', ',');
num_trials = str2num(cell2mat(t_val{1,1})) + 1;

% Get protocol name
if(contains(lines{1,1}{protocol_database_lineno, 1}, 'nbk_shaping07'))
    protocol_name = 'nbk_shaping07';
elseif(contains(lines{1,1}{protocol_database_lineno, 1}, 'nbk_shaping06'))
...
if(strcmp(protocol_name, 'nbk_shaping07'))
    ...
    % error checking
    if(~strcmp(image_name_list{1,1}, '01_cross'))
        disp('ERROR - First image name - 01_cross - does not seem right');
    %     return;
    end

    % get start of image table
    % Get Window 2 (Target image Index)
    [linenum] = find(cellfun(@strcmp, STARTDATA, mat2cell(repmat('ImageTableStart', length(STARTDATA), 1), ones(length(STARTDATA),1))) == 1);
    text_val = lines{1,1}{linenum+10,1};
    t_count = length(strfind(text_val, ','));
    t_cell = textscan(lines{1,1}{(linenum+10),1}, ['%s', repmat('%d', 1,t_count-1),], 'Delimiter', ',');
...
rt_corr_vert_choice = rxn_time(find(((target_type == 0) | (target_type == 2)  | (target_type == 3)) & (corr_choice == 1)));
% rt_incorr_vert_choice = rxn_time(find((target_type == 0) & (corr_choice == -1)));
rt_incorr_vert_choice = rxn_time(find(((target_type == 0) | (target_type == 2)  | (target_type == 3)) & (corr_choice == -1)));
% times_missed_vert_tar = rxn_time(find((target_type == 0) & (corr_choice == -2)));
...
% times_missed_horz_tar = rxn_time(find((target_type == 1) & (corr_choice == -2)));
